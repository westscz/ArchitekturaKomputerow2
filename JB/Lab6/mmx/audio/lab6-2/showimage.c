...
extern char* _negative(char *bitmap, unsigned int rozmiar);
...
void Filter(unsigned char * buf, int width,int height,int size,char bpp) {
	...
	int pictureSize = width*height*bpp;
	long long ticksStart = tick();
	buf = _negative(buf, pictureSize);


	long long ticksEnd = tick();
	long long ticks = ticksEnd-ticksStart;
	printf("Liczba cykli procesora = %llu\n", ticks);
};
